...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Api Commerce account.<br>
        ...
        &mdash; Api Commerce Team
      `
      return sendMail({ toEmail: email, subject: 'Api Commerce - Password Reset', content })
    })
